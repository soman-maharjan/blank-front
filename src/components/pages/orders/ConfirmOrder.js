        ...
        <div className="p-2 w-4/5 flex shadow">
          <div className="w-1/3 justify-start pl-5 capitalize mb-5">
            ...
            <p className="mt-5 text-left">
              {state.address.first_name} {state.address.last_name}
            </p>
            <p className="text-left">
              {state.address.house_number} {state.address.address},{" "}
              {state.address.zip_code}
            </p>
            {state.address.apartment !== null ? (
              <p className="text-left">Apartment: {state.address.apartment}</p>
            ) : null}
            {state.address.company !== null ? (
              <p className="text-left">Company: {state.address.company}</p>
            ) : null}
            <p className="text-left">
              Phone Number: {state.address.phone_number}
            </p>
            <p className="text-left">
      ...
      <div className="flex justify-center my-5">
        <div className="mt-10 w-3/5">
          ...
          <div className="flex mt-10 mb-5 bg-gray-100 -mx-8 p-2">
            <h3 className="font-semibold text-gray-600 uppercase w-2/4">
              Product Details
            </h3>
            <h3 className="font-semibold text-gray-600 uppercase w-1/4 text-center">
              Quantity
            </h3>
            <h3 className="font-semibold text-gray-600 uppercase w-1/4 text-center">
              Total
          ...
          <p className="text-right text-lg mt-3">
            <span className="font-semibold">Total :</span> Rs {state.grandTotal}
          </p>
          <div className="justify-end flex my-5 -mx-7">
            <Link
              to="/"
              className=" btn btn-success h-12 w-52 bg-green-500 text-white border-green-500"
            >
              Continue Shopping
            </Link>
          </div>
