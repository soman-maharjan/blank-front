...
export default function OrderDetails(props) {
  ...
  useEffect(() => {
    if (order.status == "Ready To Ship") {
      setLoading(true);
      axios
        .get(`/api/pickup-address/${order._id}`)
        .then((response) => {
          setLoading(false);
      ...
      <div className="w-11/12 bg-white shadow-md mt-5 text-left p-5">
        ...
        <div className="grid grid-cols-1 mt-5 gap-4">
          <div className="grid grid-cols-2 mt-2 gap-5">
            <div>
              <label htmlFor="orderId" className="text-gray-500">
                Order Id
              </label>
              <p className="break-words">{order._id}</p>
            </div>
            <div>
              <label htmlFor="sellerSku" className="text-gray-500">
                SKU
              </label>
        ...
        {order.status == "pending" ? (
          <>
            <div className="grid grid-cols-1 mt-5 gap-5">
              <select
                name=""
                className="w-full p-2 outline-none rounded max-w-xs border-2 border-gray-400"
            ...
            <OrderAddressForm
              ...
              disable={disable}
              orderId={order._id}
              orderDetailsHandler={props.orderDetailsHandler}
            />
          </>
        ) : null}
      </div>
        ...
        <div className="w-11/12 bg-white shadow-md  text-left p-5 mb-10">
          <div className="mb-5 flex">
            <p className="font-semibold text-xl mt-5">
              PICK-UP ADDRESS (Pick-up Date : {address.pickup_date})
            </p>
          </div>
          <hr />
