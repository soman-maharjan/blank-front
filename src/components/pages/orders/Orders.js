...
class Order extends Component {
    ...
    componentDidMount() {
        axios.get('/api/seller-order')
            .then(response => {
                this.setState({...this.state, filteredData: response.data, data: response.data}, (this.initializeFuse))
            })
    ...
    value = (column, row) => {
        switch (column.id) {
            ...
            case 'options':
                return (
                    <>
                        <a onClick={() => this.props.changePage({page: 'order-details', order: row})}
