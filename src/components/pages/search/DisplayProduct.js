...
export default function DisplayProduct(props) {
  const { submitHandler, products, setProducts, filters, setFilters, value } =
    props;
  ...
  for (let x = 5; x > 0; x--) {
    data.push(
      <div
        ...
        onClick={() => setFilters({ ...filters, rating: x })}
      >
    ...
    <div className="flex mt-3 bg-grey-100">
      <div className="px-2 border-r-2 w-1/6 min-h-screen">
        ...
        <div className="border-b-2 pb-5 block">
          ...
          />
          <button
            className="ml-2 bg-indigo-500 hover:bg-indigo-700  rounded p-1 transform translate-y-1"
