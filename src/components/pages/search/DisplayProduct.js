...
export default function DisplayProduct(props) {
    ...
    for (let x = 5; x > 0; x--) {
        data.push(<div className="flex cursor-pointer" onClick={() => setFilters({...filters, rating: x})}><Rating
            name="read-only" value={x} readOnly/></div>)
    }
        ...
        <div className="flex mt-3 bg-grey-100">
            <div className="px-2 border-r-2 w-1/6">
                ...
                <div className="border-b-2 pb-5 block">
                    ...
                    <input onChange={(event) => setFilters({...filters, min: event.target.value})} value={filters.min}
                           type="number" min="0" name="min"
                           className="mt-2 w-20 h-8 border-2 border-gray-300 outline-none focus:border-indigo-600 mr-2 p-3 rounded-lg"/>
                    <input onChange={(event) => setFilters({...filters, max: event.target.value})} value={filters.max}
                           type="number" min="0" name="max"
                           className="mt-2 w-20 h-8 border-2 border-gray-300 outline-none focus:border-indigo-600 mr-2 p-3 rounded-lg"/>
                    <button className="ml-2 bg-indigo-500 hover:bg-indigo-700  rounded p-1 transform translate-y-1"
                            onClick={submitHandler}><ChevronRightIcon className="h-5 w-5 text-white"/></button>
                </div>
