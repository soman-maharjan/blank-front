    ...
    function increaseQuantity(sku, idx) {
        ...
        if (quantityValue.current[idx] !== undefined) {
            dispatch({
                ...
                sku: sku,
                quantity: parseInt(quantityValue.current[idx].current.value, 10) + 1
            })
        }
    }
    ...
    const details = products !== null ? products.map((product, idx) => (
        <div key={idx} className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
            <div className="flex w-2/5">
                <div className="w-20">
                    <img className="h-24" src={`${process.env.REACT_APP_IMAGE_URL}${product.sku.images[0]}`} alt/>
                </div>
                <div className="flex flex-col justify-between ml-4 flex-grow">
                    ...
                    <a onClick={() => removeItem(product.sku.sellerSku)}
                       className="cursor-pointer font-semibold hover:text-red-500 text-gray-500 text-xs">Remove</a>
                </div>
            </div>
            <div className="flex justify-center w-1/5">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 cursor-pointer" fill="none"
                     viewBox="0 0 24 24" stroke="currentColor"
                     onClick={() => increaseQuantity(product.sku.sellerSku, idx)}>
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                </svg>
                <input id="input-arrow" min="1" ref={quantityValue.current[idx]}
                       className="mx-2 border text-center w-14 focus:outline-none" type="number" name="quantity"
                       value={product.quantity}/>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 cursor-pointer" fill="none"
                     viewBox="0 0 24 24" stroke="currentColor"
                     onClick={() => decreaseQuantity(product.sku.sellerSku, idx)}>
                ...
                <div className="flex shadow-md my-10">
                    <div className="w-3/4 bg-white px-10 py-10">
                        ...
                        <a onClick={continueShopping}
                           className="hover:bg-gray-100 p-2 rounded cursor-pointer flex font-semibold text-indigo-600 text-sm mt-10 w-44">
                    ...
                    <div id="summary" className="w-1/4 px-8 py-10">
                        ...
                        <div>
                            ...
                            <select
                                className="block p-1 text-gray-600 w-full text-sm outline-none border-2 border-gray-300 rounded">
                        ...
                        <div className="border-t mt-40 ">
                            ...
                            <Link to='/shipping'
                                  className="btn btn-success h-12 w-52 border-green-500 bg-green-500  text-white"
                                  disabled={products.length < 1 ? true : false}>Checkout</Link>
                        </div>
                    </div>
                </div>
            </div>
