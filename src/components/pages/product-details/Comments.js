...
function Comments({ productId, user }) {
  ...
  const submit = (event) => {
    ...
    if (comment != "") {
      axios
        .post("api/comments", { comment: comment, product_id: productId })
        .then((response) => {
          setComment("");
          showNotification({
            title: "Comment Posted!",
            message:
              "The comment has been posted and is under review! It will be visible to seller after reviewing.",
            color: "green",
          });
        })
        .catch((error) => console.log(error.response));
    ...
    <div className="my-4">
      ...
      <div className="flex">
        <svg
          ...
          className="h-5 w-5 mr-5 mt-1 text-red-500"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2}
      ...
      {c.reply ? (
        <>
          <div className="flex mt-1">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5 mt-1 mr-5 text-indigo-500"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
          ...
          <p className="text-gray-400 text-sm ml-10">
            ...
            <span className="ml-4">
              {" "}
              - <Moment fromNow>{c.updated_at}</Moment>
            </span>
          </p>
        </>
      ) : (
  ...
  return (
    <div className="w-full rounded bg-white shadow-md p-5 px-10 mx-auto text-gray-800 relative md:text-left mt-4">
      <div className="items-center">
        <h6 className="text-lg font-semibold leading-normal mt-0 mb-4 border-b pb-2">
          Comments
        </h6>
            ...
            <div className="form-control">
              <form>
                <div className="relative">
                  ...
                  <button
                    onClick={submit}
                    className="absolute rounded-none top-0 right-0 rounded-l-none btn min-h-0 h-10 bg-indigo-500 hover:bg-indigo-600 border-none  rounded"
                  >
                    Post it!
                  </button>
                </div>
