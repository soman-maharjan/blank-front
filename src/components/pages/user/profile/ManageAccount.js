...
export default function ManageAccount() {
    ...
    return loading ? <Loading /> : (
        <div className="w-full bg-white h-screen">
            <div className="bg-gradient-to-r from-indigo-400 via-blue-500 to-purple-500 h-56">
            ...
            <div class="grid grid-cols-3 gap-3 w-full px-7 transform -translate-y-20">
                <div className="p-4 bg-white text-left border rounded">
                    <div className="justify-center flex">
                        <img src={process.env.REACT_APP_IMAGE_URL + user.profileImage} alt="" className="hover:opacity-90 mask mask-circle h-36" />
                    </div>
                    <p className="text-center font-bold mt-4">{user.name}</p>
                    <p className="text-center mt-3">Joined on <Moment format="YYYY/MM/DD">{user.created_at}</Moment></p>
                </div>
