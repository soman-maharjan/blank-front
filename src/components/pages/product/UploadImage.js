...
export default function UploadImage(props) {
  ...
  const onChange = (imageList) => {
    ...
    img.forEach((element) => {
      ...
      props.formData.append(
        `${props.element.sellerSku}[]`,
        element,
        uniqueFileName
      );
    });
    ...
    <>
      ...
      >
        {({
          ...
          >
            <a
              className="bg-blue-300 font-normal text-black p-1 rounded m-1 cursor-pointer btn border-none min-h-0 h-10 hover:none hover:bg-blue-500"
            ...
            &nbsp;
            <a
              className="bg-red-300 text-black font-normal p-1 rounded m-1 cursor-pointer btn border-none min-h-0 h-10 hover:none hover:bg-red-500"
      ...
      {props.error !== undefined ? (
        <div className="text-red-500">Image must be uploaded</div>
      ) : null}
    </>
  );
}
